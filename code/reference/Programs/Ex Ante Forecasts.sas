...
%let lz = nemass;
/* determine whether forecasted weather or actual weather will be used*/
%let temp = tempf1;
%let hum  = humf1;
%let cc = ccf1;
%let ws = wsf1;
...
/*fit the model based on actual weather, make forecasts using forecasted weather*/
...
/*
===============================================================================
Forecasted Z matrix based off of vendor 2's forecasts
===============================================================================
*/
data forecast_v2;
	merge ds2 knotst; /*ds2 knotst*/
...
proc summary;
	var abs_error abs_pct_error abs_squared_error;
	output out = overall_mape2(drop = _FREQ_ _TYPE_)
	   mean(abs_error)=MAE
	   mean(abs_pct_error)=MAPE
       mean(abs_squared_error)=MSD;
run;
...
proc sort data = overall_mape;
	by hour2;
run;
proc summary data = overall_mape;
by hour2;
	var abs_error abs_pct_error abs_squared_error;
	output out = overall_mape3(drop = _FREQ_ _TYPE_)
	   mean(abs_error)=MAE
       mean(abs_pct_error)=MAPE
       mean(abs_squared_error)=MSD;
run;
...
by month;
	var abs_error abs_pct_error abs_squared_error;
	output out = overall_mape4(drop = _FREQ_ _TYPE_)
	   mean(abs_error)=MAE
       mean(abs_pct_error)=MAPE
       mean(abs_squared_error)=MSD;
run;
...
data yhat1;
	set yhat;
	if date ge '17JUL10'd and date le '24JUL10'd;
	   log_load = log(load);
